...
5. run deterministic checks against the observation;
6. expose coverage limits and deterministic findings;
7. allow interpretation or remediation proposals only downstream;
8. require separate authorization before any write or remediation action.
...
This proves artifact integrity and provenance of the collected bytes. It does **not** prove that the website, operator, or business claim is truthful.
...
Use `sentinel/config.website-evidence.example.json` as the starting profile and run the normal Sentinel CLI with a state path. The CLI now creates a content-addressed evidence store at `evidence_root` (default `.sentinel/evidence`) and passes it to the HTTP observer.
...
1. CI-green evidence-store and HTTP integration tests.
2. An export/verification command that packages referenced artifacts and independently re-hashes them.
3. A versioned receipt that signs or otherwise independently authenticates the package manifest; hashes alone establish integrity, not signer identity.
4. Browser-based collection for JavaScript-rendered DOM, network waterfall, console failures, and screenshots, with the same content-addressed storage rules.
5. Deterministic adapters for established tools such as axe-core/Lighthouse and approved security scanners rather than asking an LLM to reproduce their checks.
6. Machine-enforceable authorization profiles for any active or authenticated test mode.
7. A before/after verification receipt for authorized remediation workflows.
